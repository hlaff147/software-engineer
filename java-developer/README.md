# Java Backend Developer Interview Questions

A comprehensive guide covering Java, Spring Boot, Microservices, and related technologies.

## Table of Contents

1. [Java Core](./01-java-core.md)
2. [Spring + Spring Boot](./02-spring-boot.md)
3. [Microservices](./03-microservices.md)
4. [Coding Questions](./04-coding-questions.md)
5. [Others (Kafka, Kubernetes, Docker, SQL)](./05-others.md)
6. [☕ Java & Spring Under the Hood (Perguntas Aprofundadas)](./JAVA_SPRING_UNDER_THE_HOOD.md)

